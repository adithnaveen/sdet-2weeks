...
public class DatePickerTest {
	...
	public void partialTextTest() {
		...
		List<String> listOfFlyFrom = new ArrayList<String>(); 
		List<String> listOfFlyTo = new ArrayList<String>(); 
		...
		List<WebElement> flyFromPorts = genericMethods.
					getElementAsList("results-item", "class"); 

		
		
		System.out.println("List of fly from ");
		System.out.println("___________________________________________________");
		for(WebElement temp : flyFromPorts) {
			System.out.println(temp.getText());
			listOfFlyFrom.add(temp.getText()); 
		}
		...
		List<WebElement> flyToPorts = genericMethods.
				getElementAsList("results-item", "class"); 
		
		
		System.out.println("List of fly to ");
		System.out.println("___________________________________________________");
		for(WebElement temp : flyToPorts) {
			System.out.println(temp.getText());
			listOfFlyTo.add(temp.getText()); 
		}
		
		for(String temp : listOfFlyFrom) {
			genericMethods.getElement("flight-origin-hp-flight", "id").clear();
			genericMethods.getElement("flight-origin-hp-flight", "id").sendKeys(temp);
			sleep(500);
			
			for(String temp1 : listOfFlyTo) {
				genericMethods.getElement("flight-destination-hp-flight", "id").clear();
